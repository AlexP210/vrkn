...
class _AbstractJittedTD(jit.ScriptModule):
    ...
    def _flatten(x: torch.Tensor) -> tuple[torch.Tensor, int, int]:
        ...
        new_shape = (
            [bs, x.shape[2]]
            if len(x.shape) == 3
            else [bs, x.shape[2], x.shape[3], x.shape[4]]
        )
    ...
    @staticmethod
    def _get_full(
        valid: torch.Tensor, mask: torch.Tensor, default_value: int
    ) -> torch.Tensor:
        full = torch.ones(
            size=mask.size()[:2] + valid.size()[1:],
            device=valid.device,
            dtype=valid.dtype,
        )
        full *= default_value
...
class Jitted12TD(_AbstractJittedTD):
    ...
    def forward(self, x: torch.Tensor) -> tuple[torch.Tensor, torch.Tensor]:
        ...
        y1 = (
            y1_flat
            if 0 in self._copy_through
            else self._unflatten(y1_flat, batch_size, seq_length)
        )
        y2 = (
            y2_flat
            if 1 in self._copy_through
            else self._unflatten(y2_flat, batch_size, seq_length)
        )
        return y1, y2
...
class Jitted13TD(_AbstractJittedTD):
    ...
    def forward(
        ...
        y3 = (
            y3_flat
            if 2 in self._copy_through
            else self._unflatten(y3_flat, batch_size, seq_length)
        )
        return y1, y2, y3
    ...
    def forward(self, x: torch.Tensor, mask: Optional[torch.Tensor] = None):
        if self._mask_if_possible and mask is not None:
            return self._forward_masked(
                x=x, mask=mask.reshape([x.shape[0], x.shape[1]])
            )
        else:
...
class Jitted12TDPotentiallyMasked(_AbstractJittedTDPotentiallyMasked):
    ...
    def _forward_masked(self, x: torch.Tensor, mask: torch.Tensor):
        ...
        y1 = (
            y1_flat
            if 0 in self._copy_through
            else self._get_full(y1_flat, mask, self._default_value)
        )
        y2 = (
            y2_flat
            if 1 in self._copy_through
            else self._get_full(y2_flat, mask, self._default_value)
        )
        return y1, y2
