...
class AbstractObjective(nn.Module):
    ...
    def __init__(self, model: AbstractSSM, decoder_loss_scales: list[float]):
    ...
    def compute_losses(
        ...
        return self.compute_losses_and_states(
    ...
    def mse(
        ...
        if element_wise_mask is not None:
            ...
            element_wise_mse = (
                target - predicted
            ).square() * element_wise_mask.unsqueeze(2)
            num_valid_pixels = (
                torch.sum(element_wise_mask, dim=(-2, -1)) * element_wise_mse.shape[2]
            )
        ...
        if sample_wise_mask is not None:
            ...
            return (sample_wise_mse * sample_wise_mask).sum() / torch.count_nonzero(
                sample_wise_mask
            )
        else:
    ...
    def _get_reconstruction_losses(
        ...
        for i, dec in enumerate(self._model.decoders):
            e_mask = (
                None if element_wise_loss_masks is None else element_wise_loss_masks[i]
            )
        ...
        recon_ll = torch.stack(
            [s * l for s, l in zip(self._decoder_loss_scales, reconstruction_lls)],
            dim=0,
        ).sum(dim=0)
    ...
    @staticmethod
    def _build_log_dict(
        recon_lls: list[torch.Tensor],
        recon_mses: list[torch.Tensor],
        kl_dict: dict[str, float],
    ) -> dict[str, float]:
        log_dict = OrderedDict(
            {
                "decoder_{}_ll".format(i): ll.detach().cpu().numpy()
                for i, ll in enumerate(recon_lls)
            }
        )
        mse_dict = OrderedDict(
            {
                "decoder_{}_mse".format(i): mse.detach().cpu().numpy()
                for i, mse in enumerate(recon_mses)
            }
        )
    ...
    def gaussian_ll(
        ...
        if element_wise_mask is not None:
            ...
            num_valid_pixels = (
                torch.sum(element_wise_mask, dim=(-2, -1)) * element_wise_nll.shape[2]
            )
            num_pixels = target.shape[2] * target.shape[3] * target.shape[4]
            sample_wise_nll = (
                element_wise_nll.sum(dim=(2, 3, 4)) / num_valid_pixels
            ) * num_pixels
            return sample_wise_nll.mean()
        else:
            sample_wise_nll = (norm_term + exp_term).sum(
                tuple(-(i + 1) for i in range(len(target.shape) - 2))
            )

        if sample_wise_mask is not None:
            sample_wise_mask = sample_wise_mask.reshape(sample_wise_nll.shape)
            return (sample_wise_nll * sample_wise_mask).sum() / torch.count_nonzero(
                sample_wise_mask
            )
        else:
            return sample_wise_nll.mean()
    ...
    def _compute_kl_filtered(
        ...
        return AbstractObjective._compute_kl(
            lhs_mean=lhs_mean[:, time_slice],
