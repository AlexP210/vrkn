...
class AbstractTrainer:
    ...
    def __init__(
        self, objective: AbstractObjective, model: AbstractSSM, eval_interval: int
    ):
    ...
    def train_epoch(self, data_loader: data.DataLoader) -> tuple[OrderedDict, float]:
        ...
        for i, batch in enumerate(data_loader):
            ...
            if i == 0:
                avg_log_dict = OrderedDict(
                    {k: v / batches_per_epoch for k, v in log_dict.items()}
                )
            else:
    ...
    def evaluate(self, iteration: int, data_loader: data.DataLoader) -> OrderedDict:
        ...
        if self.will_evaluate(iteration):
            ...
            for i, batch in enumerate(data_loader):
                ...
                log_dict = OrderedDict(
                    {"model/{}".format(k): v for k, v in log_dict.items()}
                )
                log_dict = OrderedDict(
                    {"model/neg_elbo": loss.detach_().cpu().numpy(), **log_dict}
                )
                if i == 0:
                    avg_log_dict = OrderedDict(
                        {k: v / batches_per_epoch for k, v in log_dict.items()}
