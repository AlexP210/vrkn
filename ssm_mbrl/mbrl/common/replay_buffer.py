...
class ReplayBuffer:
    ...
    def save_normalization_parameters(
        self, obs_means: list[torch.Tensor], obs_stds: list[torch.Tensor]
    ):
    ...
    def _collect_values(
        self, use_keys: Optional[list[str]] = None
    ...
    def get_data_loader(
        ...
        if loss_masks is None:
            ...
            loss_masks = [
                (
                    None
                    if loss_masks[0][i] is None
                    else torch.stack([lm[i] for lm in loss_masks], dim=0)
                )
        ...
        if obs_valid is None:
            ...
            obs_valid = [
                torch.stack([ov[i] for ov in obs_valid], dim=0)
                for i in range(len(obs_valid[0]))
            ]
    ...
    def _sample_seqs(
        ...
        for si in seq_idx:
            start_idx = np.random.randint(
                low=0, high=all_actions[si].shape[0] - seq_length
            )
            ...
            if all_loss_masks is not None:
                loss_masks.append(
                    [
                        None if lm is None else lm[time_slice]
                        for lm in all_loss_masks[si]
                    ]
                )
            if all_obs_valid is not None:
                obs_valid.append(
                    [None if ov is None else ov[time_slice] for ov in all_obs_valid[si]]
                )
    ...
    def _collate_fn(
        ...
        prev_action_batch = data_batch[idx_dict["actions"]].to(
        ...
        if self._has_loss_masks:
            ...
            for i in range(num_obs + 1):
                ...
                if k in idx_dict.keys():
                    loss_masks.append(
                        data_batch[idx_dict[k]].to(device, non_blocking=True)
                    )
                else:
