...
class MBRLFactory:
    ...
    @staticmethod
    def build_env_and_replay_buffer(
        env, img_preprocessor: ImgPreprocessor, config: ConfigDict
    ):

        with torch.inference_mode():
            replay_buffer = ReplayBuffer(
                add_reward_to_obs=False,
            ...
            initial_obs, initial_acts, initial_rewards, initial_infos = (
                MBRLFactory.collect_initial_data(
                    env=env,
            ...
            if config.mbrl_exp.normalize_obs and not all(env.obs_are_images):
                obs_means, obs_stds = MBRLFactory._inplace_normalize(
                    inputs=initial_obs, are_images=env.obs_are_images
                )
                env = ObsNormalizationEnvWrapper(
                    env=env, obs_means=obs_means, obs_stds=obs_stds
                )
                replay_buffer.save_normalization_parameters(
                    obs_means=obs_means, obs_stds=obs_stds
                )
            replay_buffer.add_data(
                key="initial",
    ...
    @staticmethod
    def build_data_collector(env, policy: AbstractPolicy, config: ConfigDict):
    ...
    @staticmethod
    def _inplace_normalize(inputs: list[list[torch.Tensor]], are_images: list[bool]):
        means, stds = [], []
        for i in range(len(inputs[0])):
            if are_images[i]:
                ...
                s, m = torch.std_mean(
                    torch.cat([obs[i] for obs in inputs], dim=0), dim=0
                )
                if (s < 1e-2).any():
    ...
    @staticmethod
    def _rollout(env, sequence_length: int):
    ...
    @staticmethod
    def collect_initial_data(env, num_sequences: int, sequence_length: int = -1):
        t0 = time.time()
        with torch.inference_mode():
            all_observations, all_actions, all_rewards, all_infos = [], [], [], []

            for i in range(num_sequences):
                observations, actions, rewards, infos = MBRLFactory._rollout(
                    env=env, sequence_length=sequence_length
                )
                no = len(observations[0])
                all_observations.append(
                    [
                        torch.stack([o[i] for o in observations], dim=0)
                        for i in range(no)
                    ]
                )
