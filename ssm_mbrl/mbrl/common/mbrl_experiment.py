...
class MBRLExperiment:
    ...
    def build(
        ...
        if self._verbose:
            print("=== Environment ===")
            print(env_config)
        base_env = self._env_factory.build(seed=self._seed, config=env_config)
        ...
        if any(base_env.obs_are_images):
            img_preprocessor = ImgPreprocessor(
                depth_bits=mbrl_config.img_preprocessing.color_depth_bits,
                add_cb_noise=mbrl_config.img_preprocessing.add_cb_noise,
            )
        else:
            img_preprocessor = None
        self._env, self._replay_buffer = self._mbrl_factory.build_env_and_replay_buffer(
            env=base_env, img_preprocessor=img_preprocessor, config=mbrl_config
        )
        self._rb_kwargs_train = {
            "seq_length": mbrl_config.mbrl_exp.model_updt_seq_length,
        ...
        obs_sizes = [
            o.shape[0] if len(o.shape) == 1 else o.shape
            for o in self._env.observation_space
        ]
        self._model = self._model_factory.build(
            config=model_config,
        ...
        self._policy = self._policy_factory.build(
            model=self._model,
        ...
        self._data_collector = self._mbrl_factory.build_data_collector(
            env=self._env, policy=self._policy, config=mbrl_config
        )
        ...
        self._trainer = self._trainer_factory.build(
            policy=self._policy, model=self._model, config=trainer_config
        )
        ...
        if self._policy_eval_factories is not None:
            ...
            for factory in self._policy_eval_factories:
                ...
                evaluator = factory.build(
                    env=self._env, policy=self._policy, config=config
                )
                if self._verbose:
    ...
    def iterate(self, iteration: int):
        ...
        train_loader = self._replay_buffer.get_data_loader(
            device=self._device, **self._rb_kwargs_train
        )
        train_dict, train_time = self._trainer.train_epoch(data_loader=train_loader)
        with torch.no_grad():
            log_dict = self._model_log(model_log_dict=train_dict, time=train_time)

            log_dict, val_loader = self._eval_model(
                iteration=iteration, log_dict=log_dict
            )
            log_dict = self._collect_new_data(iteration=iteration, log_dict=log_dict)
            log_dict = self._eval_policy(iteration=iteration, log_dict=log_dict)
            return log_dict

    def _eval_model(self, iteration: int, log_dict: OrderedDict):
        if self._trainer.will_evaluate(iteration):
            val_loader = self._replay_buffer.get_data_loader(
                device=self._device, **self._rb_kwargs_val
            )
        else:
            val_loader = None
        val_dict = self._trainer.evaluate(data_loader=val_loader, iteration=iteration)
        log_dict = self._model_log(
            model_log_dict=val_dict, time=None, training=False, log_dict=log_dict
        )
        if self._model_evaluators is not None:
            eval_results = []
            for evaluator in self._model_evaluators:
                eval_results.append(
                    evaluator.evaluate(data_loader=val_loader, iteration=iteration)
                )
            log_dict = self._log_evaluators(
    ...
    def _eval_policy(self, iteration: int, log_dict: OrderedDict):
    ...
    def _collect_new_data(self, iteration: int, log_dict: OrderedDict):

        assert not self._replay_buffer.is_frozen
        observations, actions, rewards, infos, collect_time = (
            self._data_collector.collect()
        )
        avg_reward = sum([torch.sum(r).detach().numpy() for r in rewards]) / len(
            rewards
        )
        avg_seq_length = sum([len(r) for r in rewards]) / len(rewards)
        log_dict = self._log_collection(
        ...
        self._replay_buffer.add_data(
            key="iter{:04d}".format(iteration),
    ...
    def log_str_form_kv_pair(k, v) -> str:
        if isinstance(v, float):
            ...
            log_str = "{}: {} ".format(
                k, np.array2string(v, precision=5, max_line_width=int(1e300))
            )
        else:
    ...
    def _log_collection(
        ...
        if self._verbose:
            collect_log_str = "Data Collection: Collected {:03d} Sequence(s) ".format(
                num_seqs
            )
