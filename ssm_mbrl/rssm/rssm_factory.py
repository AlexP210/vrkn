...
class RSSMFactory:
    ...
    def __init__(self, encoder_factories, decoder_factories):
    ...
    def get_default_config(self, finalize_adding: bool = True) -> ConfigDict:
        ...
        config.add_subconf(
            name="transition",
            sub_conf=TransitionFactory.get_default_config(
                finalize_adding=finalize_adding
            ),
        )
    ...
    def _build(
        ...
        for i, (input_size, factory) in enumerate(
            zip(input_sizes, self._encoder_factories)
        ):
            current_config = getattr(config, "encoder{}".format(i))
            enc, enc_out_size = factory.build(
                input_size=input_size, config=current_config
            )
            encoders.append(
                td.Jitted11TDPotentiallyMasked(
                    base_module=enc, mask_if_possible=current_config.mask_if_possible
                )
            )
        ...
        if config.smoothing_rssm:
            ...
            for eos in enc_out_sizes:
                smoothing_preprocessors.append(
                    SmoothPreprocessor(
                        obs_dim=eos,
        ...
        transition_model = TransitionFactory().build(
            config.transition,
        ...
        for i, (output_size, factory) in enumerate(
            zip(output_sizes, self._decoder_factories)
        ):
            current_config = getattr(config, "decoder{}".format(i))
            dec = factory.build(
                input_size=transition_model.feature_size,
                output_size=output_size,
                config=current_config,
            )
            decoders.append(
                td.Jitted12TDPotentiallyMasked(
                    base_module=dec, mask_if_possible=current_config.mask_if_possible
                )
            )
    ...
    def build(
        ...
        encoders, transition_model, decoders, smoothing_preprocessors = self._build(
            config=config,
