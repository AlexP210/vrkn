...
class RRSSMTM(AbstractRSSMTM):
    ...
    def __init__(
        ...
        super(RRSSMTM, self).__init__(
            action_dim=action_dim, build_with_obs_valid=build_with_obs_valid
        )
    ...
    def _build_predict(
        ...
        pre_layers, pre_hidden_out_size = dn.build_hidden_layers(
            in_features=self._state_dim + action_dim,
            layer_sizes=[hidden_size] * num_layers,
            activation=activation,
        )
        self._pred_pre_hidden_layers = nn.Sequential(*pre_layers)
        self._pred_tm_cell = nn.GRUCell(
            input_size=pre_hidden_out_size, hidden_size=self._rec_state_dim
        )

        post_layers, post_hidden_out_size = dn.build_hidden_layers(
            in_features=self._rec_state_dim,
            layer_sizes=[hidden_size] * num_layers,
            activation=activation,
        )
        post_layers.append(
            mod.SimpleGaussianParameterLayer(
                in_features=post_hidden_out_size,
    ...
    def _build_update(
        ...
        inpt_size = (
            sum(obs_sizes)
            + (len(obs_sizes) if self._built_with_obs_valid else 0)
            + self._prior_input_size
        )
        hidden_layers, hidden_out_size = dn.build_hidden_layers(
            in_features=inpt_size,
            layer_sizes=[hidden_size] * num_layers,
            activation=activation,
        )
        hidden_layers.append(
            mod.SimpleGaussianParameterLayer(
                in_features=hidden_out_size,
                distribution_dim=self._state_dim,
                min_std_or_var=min_std,
            )
        )
    ...
    @jit.script_method
    def _get_prior_input(
        self, prior_state: dict[str, torch.Tensor]
    ) -> list[torch.Tensor]:
        if self._state_part_for_update.lower() in ["e", "estem", "estimate"]:
    ...
    @jit.script_method
    def update(
        self, prior_state: dict[str, torch.Tensor], obs: list[torch.Tensor]
    ) -> dict[str, torch.Tensor]:
        pm, ps = self._updt_dist_layer(
            torch.cat(obs + self._get_prior_input(prior_state=prior_state), dim=-1)
        )
        return {
            "mean": pm,
            "std": ps,
    ...
    def update_with_obs_valid(
        ...
        obs = [
            o.where(ov, torch.ones_like(o) * self._default_value)
            for o, ov in zip(obs, obs_valid)
        ]
    ...
    @jit.script_method
    def predict(
        self, post_state: dict[str, torch.Tensor], action: Optional[torch.Tensor] = None
    ) -> dict[str, torch.Tensor]:
        trans_input = (
            post_state["sample"]
            if action is None
            else torch.cat([post_state["sample"], action], dim=-1)
        )
        cell_state = self._pred_tm_cell(
            self._pred_pre_hidden_layers(trans_input), post_state["gru_cell_state"]
        )
    ...
    def get_initial(self, batch_size: int) -> dict[str, torch.Tensor]:
        ...
        return {
            "mean": torch.zeros(
                size=[batch_size, self._state_dim], device=p.device, dtype=p.dtype
            ),
            ...
            "gru_cell_state": torch.zeros(
                size=[batch_size, self._rec_state_dim], device=p.device, dtype=p.dtype
            ),
        }
