...
def get_standard_ae_mujoco_factories(obs_type: str):
    if obs_type == "img":
        ...
        encoder_factories = [ImageEncoderFactory(), dn.HiddenMLPFactory()]
        decoder_factories = [
            ImageDecoderFactory(),
            dn.ConstantScalarUncertaintyDenseNetFactory(),
            dn.ConstantScalarUncertaintyDenseNetFactory(),
...
class ImageEncoderFactory:
    ...
    @staticmethod
    def build(
        input_size: tuple[int, int, int], config: ConfigDict
    ) -> Tuple[nn.Module, int]:
        ...
        return (
            nn.Sequential(
                ...
                getattr(nn, config.activation)(),
                nn.Conv2d(
                    in_channels=2 * config.base_depth,
                ...
                getattr(nn, config.activation)(),
                nn.Conv2d(
                    in_channels=4 * config.base_depth,
...
class ImageDecoderFactory:
    ...
    @staticmethod
    def build(
        input_size: int, output_size: tuple[int, int, int], config: ConfigDict
    ) -> nn.Module:
        ...
        mean_net = nn.Sequential(
            # h1
            nn.Linear(in_features=input_size, out_features=4 * 8 * config.base_depth),
            ...
            # h2
            nn.ConvTranspose2d(
                in_channels=4 * 8 * config.base_depth,
                out_channels=4 * config.base_depth,
                kernel_size=(5, 5),
                stride=(2, 2),
            ),
            ...
            # h5
            nn.ConvTranspose2d(
                in_channels=1 * config.base_depth,
                out_channels=3,
                kernel_size=(6, 6),
                stride=(2, 2),
            ),
        )
        return ConstantScalarUncertaintyNet(
            mean_net=mean_net, uncertainty_val=config.output_std
        )
